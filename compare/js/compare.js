var compareApp = angular.module('compareApp', ['ngRoute', 'ngProgress']);
...
compareApp.filter('testTitle', function() {
  return function(input, mod) {
    if (input) {
      var testName = input.split("*[data-test-name=")[1].split("]")[0].replace(/["']*/g, '');
...
compareApp.controller('MainCtrl', function ($scope, $route, $routeParams, $q, $http, $filter, $location, $anchorScroll, Directories, ngProgressFactory, $timeout) {
  ...
  var testPairObj = function(a, b, c, o, fileName, testStatus){
    ...
    $http.get('./config.json')
      .success(function(data, status) {
        ...
        data.testPairs.forEach(function(o,i,a){
          ...
          $scope.testPairs.push(new testPairObj(
            '../'+o.local_reference,
            '../'+o.local_test,
            '../'+o.local_diff,
            o,
